...
<div style="width:1490px; margin-left:auto; margin-right:auto; ">
    ...
    <ul>
        ...
        <li><ul class="legal-menu nav nav-list collapse in">
             <li class="active" ><a href="add_test.php"><span class="fa fa-caret-right"></span> Prepare Test</a></li>
   ...
   <div class="content">
        <div class="header">
            ...
            <h1 class="page-title">Prepare Test Here</h1>
                    <ul class="breadcrumb">
...
<ul class="nav nav-tabs">
  <li class="active"><a href="#home" data-toggle="tab">Prepare Test here </a></li>
...
$qry="SELECT subject.* , teacher_conductor.* FROM subject
inner join teacher_conductor on teacher_conductor.subject_id = subject.subject_id
WHERE teacher_conductor.user = '$id'";
     $run=mysql_query($qry);
	...
	echo '<option  value='.$row['subject_id'].'>'.$row['name'].'</option>';
	 ...
	 ?>
        ...
        <div class="form-group">
        <label style="font-size:20px;">Test  Name</label>
        <input type="text" name="name"  class="form-control" placeholder="Test Name" required/>
        </div>
         <label style="font-size:20px;">Test Description</label>
          <textarea value="Smith" name="description" rows="6" class="form-control" placeholder="Test Description" required/></textarea>
        </div>
        ...
        <input type="text" name="duration"  class="form-control" placeholder="Duration" required/>
		 ...
		 <div class="form-group">
        <label style="font-size:20px;">Test From</label>
        <input type="date" name="from" id="from"  class="form-control" placeholder="Time  From" required/>
        </div>
        
          
         <div class="form-group">
        <label style="font-size:20px;">Test To</label>
        <input type="date" name="to"  id="to" class="form-control" placeholder="To" required/>
        </div>
         <div class="form-group">
        <label style="font-size:20px;">Secret  Code</label>
        <input type="text"  name="code" class="form-control" placeholder="Test Secret Code" required/>
        </div>
    ...
    <div class="btn-toolbar list-toolbar" style="margin-left:1200px; margin-top:10px;">
...
$duration =$_POST['duration'];
  ...
  $code =$_POST['code'];
   ...
   $from = $_POST['from'] . " " . date("H:i:s");
    $to = $_POST['to'] . " 23:59:59";
    if (strtotime($from) > strtotime($to) || strtotime($from) < (time() - 3600)) {
        
  
   echo "<script>alert('End Date is Grater then or Equal to Start Date Only')</script>";
exit();
    } else if ((strtotime($to) - strtotime($from)) <= 3600 * 24) {
        $noerror = true;
        $_GLOBALS['message'] = "Note:<br/>The test is valid upto " . date(DATE_RFC850, strtotime($to));
    }

 $qry= "INSERT INTO `test`(`t_name`, `t_description`,`t_from`,`t_to` ,`duration`, `t_code`, `subject_id`) VALUES ('$name', '$description','$from','$to','$duration', '$code', '$subject')";
mysql_query($qry);
